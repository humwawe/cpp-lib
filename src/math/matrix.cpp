...
struct matrix {
  ...
  matrix(const vector<vector<T>> &a) : n(a.size()), m(a[0].size()) {
    data.resize(n * m);
    for (int i = 0; i < n; i++) {
  ...
  template<typename int_t>
  matrix &operator ^=(int_t x) { return *this = *this ^ x; }
  ...
  pair<bool, matrix> inv() {
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (int j = 0; j < n; j++) swap(r[i][j], r[id][j]);
      auto t = T(1) / a[i][i];
  ...
  pair<bool, matrix> inv2() {
    ...
    for (int i = swaps.size(); i--;)
      for (int j = 0; j < n; j++) swap(a[j][swaps[i].first], a[j][swaps[i].second]);
    return {true, a};
  }
